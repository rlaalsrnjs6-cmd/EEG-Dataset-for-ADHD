...
# 실사용 테스트 아이디 제거
real_ids = id_class_df[id_class_df["Class"] == 1]["ID"].iloc[:1]
# 실사용 제외 분리
adhd_grops = id_class_df[(id_class_df["Class"] == 1) & (~id_class_df["ID"].isin(real_ids))]["ID"]
control_grops = id_class_df[(id_class_df["Class"] == 0) & (~id_class_df["ID"].isin(real_ids))]["ID"]
...
# zip덕분에 [0] [0] [0] 처럼 해당하는 인덱스끼리 비교가능
for window, label, current_id in zip(window_list, label_list, id_list):
    ...
    # 실사용 테스트 저장
    if current_id in real_ids.values:
...
cnn_preprocess_path = Path("logs/cnn_preprocess_logs.csv")

cnn_preprocess_record = {
    "run_time": datetime.now(ZoneInfo("Asia/Seoul")).strftime("%Y-%m-%d %H:%M:%S"),
    "input_file": "data/raw/adhd_data.csv",
    "window_size": window_size,
    "channel_count": len(eeg_channels),
    "random_seed": random_state,

    "raw_row_count": len(raw_df),
    "after_duplicate_count": len(drop_data),
    "after_null_count": len(null_data),
    "after_zero_remove_count": len(eeg_data),

    "train_window_count": len(train_windows),
    "validation_window_count": len(validation_windows),
    "test_window_count": len(test_windows),
    "real_window_count": len(real_test_windows),

    "train_id_count": len(train_ids),
    "validation_id_count": len(validation_ids),
    "test_id_count": len(test_ids),
    "real_id_count": len(real_ids), 

    "output_file": "data/cnn_processed/cnn_data.npz",
    "note": "ID 단위 70/15/15 분할"
}

save_log(cnn_preprocess_record, cnn_preprocess_path)
